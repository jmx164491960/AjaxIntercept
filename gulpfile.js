...
gulp.task("webpack", function( /*callback*/ ) {
    webpack({
        entry: './src/ajaxIntercept.js',
        output: {
            path: './dist',
            filename: 'ajaxIntercept.js',
            library: 'AjaxIntercept',
            libraryTarget: 'umd'
        }
    }, function(err /*, stats*/ ) {
...
gulp.task('mocha', function() {
    return gulp.src('test/test.ajaxIntercept.html')
        .pipe(mochaPhantomJS({
            reporter: 'spec'
        }))
})
...
gulp.task('istanbulForMochaPhantomJS', function(cb) {
    gulp.src(['dist/ajaxIntercept.js'])
        .pipe(istanbul()) // Covering files
        .pipe(istanbul.hookRequire()) // Force `require` to return covered files
        .on('finish', function() {
            gulp.src(['test/test.ajaxIntercept.html'])
                .pipe(mochaPhantomJS({
                    reporter: 'spec'
                }))
                .pipe(istanbul.writeReports()) // Creating the reports after tests runned
